...
class CustomInput extends Component{
    ...
    _is_dropdown(){
        if(this.icon && this.text){
            ...
            this.dropdown_content.style.paddingTop = 'var(--global-padding)';
        }
    ...
    _append_dropdown_items(){
        if(this._items){
            for(var i = 0; i < this._items.length; i++){
                ...
                var el = document.createElement('custom-input');
                el.setAttribute('type', 'button');
                el.setAttribute('text', item);
                el.setAttribute('item_number', i);
                el.setAttribute('icon', 'info');
                el.style.background = 'var(--theme-background-color)';
            ...
            var els = this.dropdown_content.getElementsByTagName('custom-input')
            for(var e in els){
                e = els[e]
                if(typeof e !== 'function' && typeof e !== 'number') { 
                    console.log('element')
                    console.log(e)
                    e.resizeComponents(true)
                }
            }
        }
    }
